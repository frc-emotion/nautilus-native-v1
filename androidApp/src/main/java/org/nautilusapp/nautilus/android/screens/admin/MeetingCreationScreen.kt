...
fun MeetingCreationScreen(
    ...
    val initMs = LocalDateTime.of(LocalDate.now(), LocalTime.of(initialHour, initialMinute)).toInstant(offset).toEpochMilli()
    ...
    fun TimeInputDialog(
        ...
        val (edit, init, setEdit, setTime) = when(timeType) {
            TimeType.START -> Quad(editStartTime, startTimeMs, { b: Boolean -> editStartTime = b }, {time: Long -> startTimeMs = time})
            TimeType.END -> Quad(editEndTime, endTimeMs, { b: Boolean -> editEndTime = b }, {time: Long -> endTimeMs = time; customEndTimeSet = true })
        }
        val initDateMs = if(timeType == TimeType.START) LocalDateTime.now().toInstant(ZoneOffset.UTC).toEpochMilli() else LocalDateTime.now().toInstant(ZoneOffset.UTC).toEpochMilli() + meetingValue * HOURS_TO_SECONDS * 1000
        val zOff = ZoneOffset.systemDefault().rules.getOffset(LocalDateTime.now(ZoneOffset.UTC).toInstant(ZoneOffset.UTC))
        val initDateTime = LocalDateTime.ofEpochSecond(init / 1000, 0, zOff)
        if (edit) {
            val timeState = rememberTimePickerState(initialHour = initDateTime.hour, initialMinute = initDateTime.minute)
                ...
                Card(
                    ...
                    Column(
                        Modifier
                            ...
                            .fillMaxWidth()) {
                        Text("${timeType.name.toCapitalized()} Time", style = MaterialTheme.typography.titleLarge)
                        ...
                        Row {
                            TextButton(onClick = { setTime(dateTimeStateToEpochMs(dateState, timeState)); setEdit(false) }) {
                                Text(text = "Confirm")
                            }
                            if(timeType == TimeType.END) {
                                TextButton(onClick = { customEndTimeSet = false; setEdit(false) }) {
                                    Text(text = "Auto", color = MaterialTheme.colorScheme.secondary)
                                }
                            }
