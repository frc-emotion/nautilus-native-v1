...
//    }
    ...
    Screen(onRefresh = primaryViewModel::syncMe, snack = snack) {
            ...
            tagData?.let { data ->
                coroutineScope.launch {
                    primaryViewModel.attendMeeting(
                        data,
                        {
                            showFailureDialog = true; failureDialogText = it; nfcViewmodel.setNdef(
                            null
                        )
                        },
        ...
        if (showSuccessDialog) {
            AlertDialog(
                ...
                text = { Text("Attendance logged successfully") })
        }
        if (showFailureDialog) {
            AlertDialog(
                ...
                title = { Text("Error") },
                text = { Text("Something went wrong logging attendance\n $failureDialogText") })
        }
    }
}