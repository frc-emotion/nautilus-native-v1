...
@Composable
fun SettingsScreen(viewModel: MainViewModel, snack: SnackbarHostState) {
    Screen(onRefresh = { viewModel.coroutineSync().allUnprotected }, snack = snack) {
        when (authState(viewModel.user)) {
            AuthState.NOT_LOGGED_IN -> NotLoggedInScreen(
